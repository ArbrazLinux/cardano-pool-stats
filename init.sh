...
currentEpoch=$(cardano-cli query tip --mainnet | jq -r '.epoch')

if [[ ! -d "data"  ]]
	then
		mkdir data
fi
if [[ -f "data/blocksList.json" ]]
	then
		rm data/blocksList.json
fi
if [[ -f "data/gone_delegators.json" ]]
	then
		rm data/gone_delegators.json
fi
if [[ -f "data/pool_data.json" ]]
	then
		rm data/pool_data.json
fi
